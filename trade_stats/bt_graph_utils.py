...
def graph_regime_combo(
    ...
    try:
        if pd.notnull(rg):
            ...
            if df[hi].count() > 0:
                ax1.plot(
                    df.index, df[hi], ".", color="g", label="swing high", alpha=0.6
                )
            if df[slo].count() > 0:
                ax1.plot(
                    df.index, df[slo], "o", color="r", label="swing low", alpha=0.8
                )
            if df[shi].count() > 0:
                ax1.plot(
                    df.index, df[shi], "o", color="g", label="swing high", alpha=0.8
                )
            if df[flr].count() > 0:
        ...
        if np.sum(ma_st) > 0:
            ...
            if pd.notnull(rg):  # floor/ceiling regime present
                ...
                ax1.fill_between(
                    ...
                    close,
                    ma_mt,
                    where=((regime == 1) & (ma_mt >= ma_lt) & (ma_st >= ma_mt)),
                    facecolor="green",
                    alpha=0.5,
                )
                ax1.fill_between(
                ...
                ax1.fill_between(
                    ...
                    close,
                    ma_mt,
                    where=((ma_mt >= ma_lt) & (ma_st >= ma_mt) & (close < ma_mt)),
                    facecolor="darkgreen",
                    alpha=1,
                )
                ax1.fill_between(
