...
def ma_crossover(slow_ma, fast_ma):
    ...
    crosses = diff_from_fast[
        (prev_diff < 0 < diff_from_fast) | (diff_from_fast < 0 < prev_diff)
    ]
...
def relative_series(
    base_df: pd.DataFrame, bench_df: pd.DataFrame, forex_df=None, decimal=2
) -> pd.DataFrame:
    """
    my relative series
...
def sma(df: pd.DataFrame, price, ma_per, min_per, decimals: Union[int, None]):
    """
...
def ema(df: pd.DataFrame, price: str, ma_per: int, min_per, decimals: int):
    """
...
def regime_sma(df: pd.DataFrame, price: str, short_term: int, long_term: int):
    """
...
def regime_ema(
    df: pd.DataFrame, price: str, short_term: int, long_term: int
) -> pd.DataFrame:
    """
...
def regime_breakout_breakdown(
    breakout_period: int, df: pd.DataFrame, high: str, low: str
) -> pd.DataFrame:
    """
    ...
    # define rolling high/low
    rolling_high = (
        df[high].rolling(window=breakout_period, min_periods=breakout_period).max()
    )  #
    rolling_low = (
        df[low].rolling(window=breakout_period, min_periods=breakout_period).min()
    )
...
# Calculate swings
def swings(
    df: pd.DataFrame, high: str, low: str, arg_rel_window: int, prefix: str = "sw"
) -> pd.DataFrame:
    """
    ...
    if pd.isnull(last_slo_dt) or pd.isnull(last_shi_dt):
        raise NoSwingsError(
            "Quit swing calc. [Swing High > 0 AND Swing Low] condition fails."
        )
...
def regime_fc(
    ...
    regime[stdev] = rolling_stdev(
        price_col=df[close], window=st_dev_window, min_periods=1, decimals=decimals
    )
    ...
    for i in range(1, len(regime)):
        ...
        if regime[swing_high][i] > 0:  # ignores swing lows
            top = regime[floor_ix : regime.index[i]][
                swing_high
            ].max()  # highest swing high from range[floor_ix:swing[i]]
            ceiling_test = round(
        ...
        if regime[swing_low][i] > 0:  # ignores swing highs
            bottom = regime[ceiling_ix : regime.index[i]][
                swing_low
            ].min()  # lowest swing low from ceiling
            floor_test = round(
...
def rolling_stdev(price_col: pd.Series, window: int, min_periods: int, decimals: int):
    """
    Rolling volatility rounded at 'decimals', starting at percentage 'min_per' of window 'window'
    """
    return round(
        price_col.rolling(
            window=window, min_periods=int(round(window * min_periods, 0))
        ).std(ddof=0),
        decimals,
    )
...
def signal_fcstmt(
    regime: pd.Series, st: pd.Series, mt: pd.Series, side: Side = None
) -> pd.Series:
    """
...
def stop_loss(signal: pd.Series, close: pd.Series, s_low: pd.Series, s_high: pd.Series):
    """
...
def transaction_costs(
    data: pd.DataFrame, position_column: str, daily_return, transaction_cost: float
):
    """
    ...
    data[daily_return] = np.where(
        inout, data[daily_return] - float(transaction_cost), data[daily_return]
    )
...
def init_fc_signal_stoploss(
    ...
    stats_out = stats.sort_values(by="risk_adjusted_returns").dropna(
        subset=["risk_adjusted_returns"]
    )
    # TODO include interval in name
    try:

        stats_out.reset_index().to_feather(
            fr"C:\algo_data_store\strategy_data\strategy_scoreboard\{symbol}.ftr"
        )
    except Exception as e:
        print(f"TODO catch exception {e}")
    try:
        best_stats.reset_index().to_feather(
            fr"C:\algo_data_store\strategy_data\full_stats\{symbol}.ftr"
        )
    except Exception as e:
        print(f"TODO catch exception {e}")

    return (high_score, stats)
...
def calc_stats(
    ...
    pr_expanding = ts2.profit_ratio(profits=profits_expanding, losses=losses_expanding)

    profits_roll = ts2.rolling_profits(strategy_returns_1d, window)
    losses_roll = ts2.rolling_losses(strategy_returns_1d, window)
    pr_roll = ts2.profit_ratio(profits=profits_roll, losses=losses_roll)
...
def single_init_fc_signal_stoploss(
    ...
    r_st_ma = sma(
        df=data, price=relative_close, ma_per=st_ma_per, min_per=1, decimals=None
    )
    data["st_ma"] = r_st_ma

    r_mt_ma = sma(
        df=data, price=relative_close, ma_per=mt_ma_per, min_per=1, decimals=None
    )
    data["mt_ma"] = r_mt_ma
    ...
    (
        ...
        data["signal"],
        data["stop_status"],
    ) = data.stop_losses.trail_to_cost(trail_stop=data.trail_stop)
    ...
    (
        data["size_remaining"],
        data["target_price"],
    ) = data.signals.init_simulated_scale_out(
...
if __name__ == "__main__":
    LocalClient.price_history(
        "FNILX", freq_range=tdargs.freqs.day.range(tdargs.periods.y2, datetime.today())
    )
