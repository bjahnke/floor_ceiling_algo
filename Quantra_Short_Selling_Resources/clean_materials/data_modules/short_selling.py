...
def ema(df,price,ma_per,min_per,decimals):
    ...
    '''
    ema = round(df[price].ewm(span= ma_per,min_periods = int(round(ma_per*min_per,0))).mean(),decimals)
    return ema
...
def regime_sma(df,price,short_term,long_term):
    ...
    df['regimes_sma'+'_'+str(short_term)+'_'+str(long_term)] = df['regime_sma'+'_'+str(short_term)+'_'+str(long_term)].fillna(method='ffill')
    return (df)
...
def regime_ema(df,price,short_term,long_term):
    ...
    df['regime_ema'+'_'+str(short_term)+'_'+str(long_term)] = df['regime_ema'+str(short_term)+str(long_term)].fillna(method='ffill')
    return (df)
...
def regime_breakout_breakdown(breakout_period,df,high,low):
    ...
    df['regime_breakout_breakdown'+'_'+str(breakout_period)] = df['regime_breakout_breakdown'+'_'+str(breakout_period)].fillna(method='ffill')
    return (df)
...
def regime_fc(df,close,swing_low,swing_high,threshold,t_dev,decimals):
    ...
    stdev = 'stdev'
    regime[stdev] = rolling_stdev(df=df,price=close,t_dev=t_dev,min_per=1,decimals=decimals)
    ...
    if str(close)[0] == 'r': # a) test the first letter of the cl input variable
        ...
        rg_cols = ['r_floor','r_ceiling','r_regime_change','r_regime_floorceiling','r_floorceiling','r_regime_breakout']
    else: # absolute series
        rg_cols = ['floor','ceiling','regime_change','regime_floorceiling','floorceiling','regime_breakout']
    ...
    regime = pd.concat([regime, # existing df
        #temporary df with same index, regime columns initialised at nan 
         pd.DataFrame(np.nan, index=regime.index, columns= rg_cols)], # temp df
        axis=1) # along the vertical axis

    # c) column variables names instantiation via a list comprehension 
    floor, ceiling, regime_change, regime_floorceiling, floorceiling,regime_breakout =  [list(rg_cols)[n] for n in range(len(rg_cols))]
    ...
    df[regime_change] = np.where(np.sign(df[regime_floorceiling]*df[regime_breakout])==-1,df[floorceiling],df[regime_change]) # re-assign fc
    df[regime_floorceiling] = np.where(np.sign(df[regime_floorceiling]*df[regime_breakout])==-1,df[regime_breakout],df[regime_floorceiling]) # rgme chg
...
def rolling_stdev(df,price,t_dev,min_per,decimals):
    ...
    stdev = round(df[price].rolling(window= t_dev,min_periods= int(round(t_dev*min_per,0))).std(ddof=0),decimals)
    return stdev
